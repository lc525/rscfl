...
\pname{ } allows the user to pre-define the subsystem boundries they are
...
required to provide a definition of the expected system behaviour, in order to
...
Other systems such as \cite{fay} go some way towards providing a programatic
interface to trace and profile data, but are still limited to a post-hoc
evaluation. \pname{}'s API allows trace data to be retireved and evaluated in
line with the target processes execution. This gives the programmer much
broader knowledge of how the kernel is behaving under their program and allows
them to make more informed decisions.

One useful feature to have when designing a dependible system is being able to
...
provided with the results of a what-if senario.
...
%X-Ray, GWP, Weatherman, Nirvana & iDNA
