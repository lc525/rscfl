...
Virtualization has become the norm for increasing server utilization in modern
datacenters. However, multiplexing numerous services on the same physical host
(either by running processes, OS containers or full-fledged VMs) means that the
chances of experiencing high contention on particular resources grow. This
translates into more variability in system behavior, and less predictability in
terms of performance and failure rates \cite{?}.
...
other processes sharing the same resources. Those side effects could manifest as
performance degradation (for example, in cache trashing and IO storms) or
changes in the output of the system (fewer results returned, loss of precision
if running timeout-based processing algorithms).
...
those implicit interactions. To this end, we are introducing \pname, a framework that provides configurable
resource utilization measurements to applications interested in monitoring their
footprint in the context of overall resource consumption
...
react to concurrent workloads (for example, by throttling or delaying their own operations). 
