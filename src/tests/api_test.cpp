...
TEST_F(APITest, UserGetSubsysforOneAcct)
{
...
TEST_F(APITest, SubsysIndexAndIdArraysAreConsistent)
{
  int no_subsys_in_idx = 0;
  one_acct_ = rscfl_get_subsys(rhdl_, &acct_);
  ASSERT_TRUE(one_acct_ != NULL);
...
TEST_F(APITest, UserAcctAggregators)
{
...
TEST_F(APITest, ReduceAPIProducesCorrectResults)
{
  ru64 kernel_cycles_;
  ...
  r_err = REDUCE_SUBSYS(
      rint, rhdl_, &acct_, 0, &kernel_cycles_r_,
