...
class SchedTest : public testing::Test
{
  protected:
    virtual void SetUp()
    {
      ...
      EXPECT_GT(sockfd_, 0);
      clock_gettime(CLOCK_MONOTONIC_RAW, &run_time);
      ...
      sub_set = rscfl_get_subsys(rhdl_, &acct_);
      ASSERT_TRUE(sub_set != NULL);

      rscfl_timespec_diff(&run_time, &start_time);
    }

    virtual void TearDown()
    {
      close(sockfd_);
      free_subsys_idx_set(sub_set);
    }

    rscfl_handle rhdl_;
    struct accounting acct_;
    int sockfd_;
    subsys_idx_set *sub_set;
...
{
  for (int i = 0; i < sub_set->set_size; i++) {
    ASSERT_GE(sub_set->set[i].sched.wct_out_local.tv_sec, 0);
    ASSERT_GE(sub_set->set[i].sched.wct_out_local.tv_nsec, 0);
  }
}
...
{
  for (int i = 0; i < sub_set->set_size; i++) {
    ASSERT_LT(sub_set->set[i].sched.wct_out_local.tv_sec, sub_set->set[i].cpu.wall_clock_time.tv_sec);
    ASSERT_LT(sub_set->set[i].sched.wct_out_local.tv_nsec, sub_set->set[i].cpu.wall_clock_time.tv_nsec);
  }
}
...
TEST_F(SchedTest, SchedTotalTimeGreater)
{
  ...
  for (int i = 0; i < sub_set->set_size; i++) {
    rscfl_timespec_add(&k_sched_time, &sub_set->set[i].sched.wct_out_local);
  }

  EXPECT_EQ(-1, rscfl_timespec_compare(&k_sched_time, &run_time));
}
